...
(defn- get-virkailija-oid-for-update-sercret
  [conn secret]
...
(defn- get-virkailija-oid-for-create-sercret
  [conn secret]
  (->> (jdbc/query conn ["SELECT virkailija_oid
...
(defn add-application [new-application applied-hakukohteet]
  (jdbc/with-db-transaction [conn {:datasource (db/get-datasource :db)}]
    ...
    (let [virkailija-oid                       (when-let [secret (:virkailija-secret new-application)]
                                                 (get-virkailija-oid-for-create-sercret conn secret))
          {:keys [id key] :as new-application} (add-new-application-version new-application true applied-hakukohteet conn)
  ...
  (jdbc/with-db-transaction [conn {:datasource (db/get-datasource :db)}]
    (let [updated-by-applicant? (not-blank? secret)
          ...
          virkailija-oid        (when-not updated-by-applicant?
                                  (get-virkailija-oid-for-update-sercret conn virkailija-secret))]
